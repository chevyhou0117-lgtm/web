...
SimulationPlan（模拟方案）
  ├── SoftConstraintConfig（软约束配置） 1:N
  ├── ParameterOverride（可调参数覆盖） 1:N
  ├── AnomalyInjection（异常注入） 1:N
  ├── WorkOrder（工单快照-业务快照） 1:N        ← 有工单时导入，可选
  ├── ProductionTask（生产任务） 1:N            ← 始终必须，wo_id 可选关联 WorkOrder
  ├── MaterialSupply（物料供应计划-业务快照） 1:N
  ├── InventorySnapshot（仓库库存快照-业务快照） 1:N
  ├── WIPBufferSnapshot（线边仓状态快照-业务快照） 1:N
  ├── DemandForecast（中长期需求预测-业务快照） 1:N  ← SMT产能规划专用
  └── SimulationResult（模拟结果） 1:1
        ├── LineBalanceResult（线平衡结果详情） 1:N（按产线）
        ├── SMTCapacityResult（SMT产能规划结果） 1:1  ← SMT产能规划专用
        │     └── SMTCapacityPeriodResult（各期明细） 1:N
        ├── SimulationStateSnapshot（逐时状态快照） 1:N
        ├── AIAnalysisResult（AI分析结果） 1:1
        │     └── ImprovementSuggestion（改善建议） 1:N
        └── PlanVersion（归档版本） N（多次保存为历史版本）

ParameterTemplate（参数模板） ← 被 SimulationPlan 引用复用
InputDataTemplate（输入数据模板） ← 被 SimulationPlan 引用复用
```
...
| **模拟方案层** | 运营模拟自建和管理的方案配置、参数覆盖、约束设置 | 运营模拟**可读写** |
| **业务数据快照层** | 来自 ERP/MES/WMS 的快照或手工导入，驱动单次仿真 | 导入后**只读**（模拟期间不再实时同步） |
| **模拟结果层** | 仿真引擎计算输出，不可手工创建 | 仿真引擎**写入**，用户**只读** |
...
```mermaid
erDiagram
    SimulationPlan ||--o{ SoftConstraintConfig : "1:N"
    SimulationPlan ||--o{ ParameterOverride : "1:N"
    SimulationPlan ||--o{ AnomalyInjection : "1:N"
    SimulationPlan ||--o{ WorkOrder : "1:N（可选）"
    SimulationPlan ||--o{ ProductionTask : "1:N"
    SimulationPlan ||--o{ MaterialSupply : "1:N"
    SimulationPlan ||--o{ InventorySnapshot : "1:N"
    SimulationPlan ||--o{ WIPBufferSnapshot : "1:N"
    SimulationPlan ||--o{ DemandForecast : "1:N（SMT专用）"
    SimulationPlan ||--o| SimulationResult : "1:1"

    WorkOrder ||--o{ ProductionTask : "1:N（可选关联）"

    SimulationResult ||--o{ LineBalanceResult : "1:N"
    SimulationResult ||--o| SMTCapacityResult : "1:1（SMT专用）"
    SimulationResult ||--o{ SimulationStateSnapshot : "1:N"
    SimulationResult ||--o| AIAnalysisResult : "1:1"
    SMTCapacityResult ||--o{ SMTCapacityPeriodResult : "1:N"
    AIAnalysisResult ||--o{ ImprovementSuggestion : "1:N"
    SimulationResult ||--o{ PlanVersion : "1:N"
    ...
    SimulationPlan {
        ...
        varchar factory_id FK
        varchar status
        json enabled_simulators
        decimal simulation_duration_hours
    }
    WorkOrder {
        varchar wo_id PK
        varchar plan_id FK
    ...
    SimulationResult {
        ...
        varchar plan_id FK
        varchar computation_status
        integer total_output
        decimal overall_lbr
        varchar bottleneck_equipment_id
    }
    LineBalanceResult {
        varchar lb_result_id PK
        varchar result_id FK
        varchar line_id FK
        decimal takt_time
        decimal lbr
        varchar bottleneck_operation_id
    }
    SMTCapacityResult {
        varchar smt_result_id PK
        varchar result_id FK
        decimal placement_rate
        integer smt_line_count
        decimal total_demand_points
    }
```
...
| line_id | VARCHAR(36) | ✓ | 所属线体（外键→ProductionLine） |
...
| standard_changeover_time | DECIMAL(10,2) | | 标准换线时间（分钟） |
...
| primary_material_type | VARCHAR(100) | | 工序处理的主要物料类型（如 PCB、锡膏SAC305、阻容、IC/接口），用于仿真物料消耗逻辑 |
...
| 字段名 | 类型 | 必填 | 说明 |
|--------|------|------|------|
| result_id | VARCHAR(36) | ✓ | 主键 |
| plan_id | VARCHAR(36) | ✓ | 所属方案（外键→SimulationPlan，1:1） |
| computation_status | VARCHAR(20) | ✓ | 计算状态：COMPUTING / SUCCESS / FAILED |
| computation_start | DATETIME | | 计算开始时间 |
| computation_end | DATETIME | | 计算结束时间 |
| total_output | INTEGER | | 总产出数量（件） |
| output_per_hour | DECIMAL(10,3) | | 每小时产出（件/小时） |
| overall_lbr | DECIMAL(5,4) | | 整体产线平衡率 LBR（0.0000~1.0000） |
| bottleneck_equipment_id | VARCHAR(36) | | 主要瓶颈设备 ID |
| bottleneck_utilization | DECIMAL(5,4) | | 瓶颈设备利用率 |
| material_shortage_count | INTEGER | | 缺料发生次数 |
| material_shortage_minutes | DECIMAL(10,2) | | 缺料导致停工累计分钟数 |
| equipment_failure_count | INTEGER | | 设备故障发生次数（启用软约束时有值） |
| equipment_downtime_minutes | DECIMAL(10,2) | | 故障停机累计分钟数 |
| result_summary | JSON | | 各产线各工序详细指标 JSON |
| error_message | TEXT | | 计算失败时的错误信息 |
| created_at | DATETIME | ✓ | 创建时间 |

#### SMTCapacityResult（SMT产能规划结果）

> **专属于 SMT 产能规划分析模拟器**。

| 字段名 | 类型 | 必填 | 说明 |
|--------|------|------|------|
| smt_result_id | VARCHAR(36) | ✓ | 主键 |
| result_id | VARCHAR(36) | ✓ | 父结果（外键→SimulationResult，1:1） |
| time_granularity | VARCHAR(10) | ✓ | 时间粒度：MONTHLY / WEEKLY |
| placement_rate | DECIMAL(5,4) | ✓ | 本次规划使用的置件率（如 0.35 = 35%） |
| smt_line_count | INTEGER | ✓ | 参与规划的 SMT 产线数量 |
| total_demand_points | DECIMAL(20,2) | | 规划周期内总置件点数需求（所有期间合计） |
| total_effective_capacity_points | DECIMAL(20,2) | | 规划周期内总有效产能点数（所有期间合计） |
| max_gap_period_label | VARCHAR(20) | | 产能缺口最大的期间标签 |
| max_gap_lines_needed | INTEGER | | 产能缺口最大期间所需新增线体数量 |
| created_at | DATETIME | ✓ | 创建时间 |

#### SMTCapacityPeriodResult（SMT产能规划各期明细）

| 字段名 | 类型 | 必填 | 说明 |
|--------|------|------|------|
| period_result_id | VARCHAR(36) | ✓ | 主键 |
| smt_result_id | VARCHAR(36) | ✓ | 所属 SMT 产能规划结果（外键→SMTCapacityResult） |
| period_label | VARCHAR(20) | ✓ | 期间标签（与 DemandForecast.period_label 对应） |
| period_start_date | DATE | ✓ | 期间起始日期 |
| period_end_date | DATE | ✓ | 期间结束日期 |
| working_days | INTEGER | ✓ | 当期工作日天数（来自工作日历） |
| demand_points | DECIMAL(20,2) | ✓ | 当期总置件点数需求（由 BOM 拆解后汇总） |
| theoretical_capacity_points | DECIMAL(20,2) | ✓ | 当期理论产能点数 = PPH × 日班次时长 × 工作日 × 产线数 |
| effective_capacity_points | DECIMAL(20,2) | ✓ | 当期有效产能点数 = 理论产能 × 置件率 |
| gap_points | DECIMAL(20,2) | ✓ | 产能差异点数 = 需求点数 - 有效产能点数；正值为缺口，负值为富余 |
| gap_lines_needed | INTEGER | ✓ | 差异线体需求数量（gap_points > 0 时向上取整；≤ 0 时为 0） |
| utilization_rate | DECIMAL(5,4) | ✓ | 产能利用率 = 需求点数 ÷ 有效产能点数 |
| period_detail | JSON | | 各产品 SMT 半成品需求量及对应点数明细 |
| created_at | DATETIME | ✓ | 创建时间 |

#### LineBalanceResult（线平衡结果）
| 字段名 | 类型 | 必填 | 说明 |
|--------|------|------|------|
| lb_result_id | VARCHAR(36) | ✓ | 主键 |
| result_id | VARCHAR(36) | ✓ | 父结果（外键→SimulationResult） |
| line_id | VARCHAR(36) | ✓ | 产线 |
| takt_time | DECIMAL(10,3) | ✓ | 目标节拍（秒，由工作日历×生产任务推算） |
| lbr | DECIMAL(5,4) | ✓ | 产线平衡率 LBR = ΣCT ÷ (瓶颈CT × 总工位数) |
| balance_loss_rate | DECIMAL(5,4) | ✓ | 平衡损失率 = 1 - LBR |
| bottleneck_operation_id | VARCHAR(36) | | 瓶颈工序 ID（CT最大） |
| bottleneck_ct | DECIMAL(10,3) | | 瓶颈工序 CT（秒） |
| idle_operation_id | VARCHAR(36) | | 空闲工序 ID（CT最小） |
| operation_load_detail | JSON | | 各工序负荷明细（工序ID→CT、利用率、与节拍偏差） |
| workshop_load_rate | DECIMAL(5,4) | | 车间负荷平衡率 |
| factory_load_rate | DECIMAL(5,4) | | 工厂负荷平衡率 |
| created_at | DATETIME | ✓ | 创建时间 |

#### SimulationStateSnapshot（仿真状态快照）
| 字段名 | 类型 | 必填 | 说明 |
|--------|------|------|------|
| snapshot_id | VARCHAR(36) | ✓ | 主键 |
| result_id | VARCHAR(36) | ✓ | 所属结果（外键→SimulationResult） |
| sim_timestamp_sec | DECIMAL(10,3) | ✓ | 仿真时间戳（秒，从 0 开始） |
| equipment_states | JSON | ✓ | 设备状态 Map（equipment_id→{status, utilization, current_product}） |
| wip_states | JSON | | 线边仓状态 Map（wip_id→{quantity, volume, fill_rate}） |
| warehouse_states | JSON | | 仓库库存 Map（warehouse_id→{material_code→available_qty}） |
| agv_states | JSON | | AGV 状态 Map（agv_id→{position, task, status}），AGV 模拟启用时有值 |
| snapshot_interval_sec | INTEGER | ✓ | 本快照的时间步长（秒，默认 60） |
| created_at | DATETIME | ✓ | 创建时间 |

> **存储策略**: 快照数据量极大，按 result_id 分区存储；方案归档后压缩降频（保留每分钟快照）。

#### AIAnalysisResult（AI分析结果）
| 字段名 | 类型 | 必填 | 说明 |
|--------|------|------|------|
| ai_result_id | VARCHAR(36) | ✓ | 主键 |
| result_id | VARCHAR(36) | ✓ | 所属仿真结果（外键→SimulationResult，1:1） |
| bottleneck_analysis | JSON | ✓ | 瓶颈分析（工位列表、根因分类、严重度评级） |
| analysis_summary | TEXT | | 综合分析摘要（自然语言，由 LLM 生成） |
| knowledge_base_version | VARCHAR(20) | | 调用的精益知识库版本 |
| model_version | VARCHAR(50) | | 调用的 AI 模型版本 |
| generation_status | VARCHAR(20) | ✓ | GENERATING / SUCCESS / FAILED |
| created_at | DATETIME | ✓ | 创建时间 |

#### ImprovementSuggestion（改善建议）
| 字段名 | 类型 | 必填 | 说明 |
|--------|------|------|------|
| suggestion_id | VARCHAR(36) | ✓ | 主键 |
| ai_result_id | VARCHAR(36) | ✓ | 所属 AI 分析结果（外键→AIAnalysisResult） |
| target_type | VARCHAR(30) | ✓ | 改善目标类型：EQUIPMENT / OPERATION / STAGE / STAFFING / LOGISTICS |
| target_id | VARCHAR(36) | | 目标对象 ID |
| suggestion_category | VARCHAR(30) | ✓ | 建议类别（见枚举 4.8） |
| priority | INTEGER | ✓ | 优先级（1=最高，5=最低） |
| description | TEXT | ✓ | 改善建议描述（具体可操作，如"贴片工序增加1台设备可将产能提升X%"） |
| expected_impact | JSON | | 预期改善效果（如 `{"lbr_improvement": 0.05, "output_increase_pct": 8}`） |
| implementation_cost | VARCHAR(20) | | 实施成本估计：LOW / MEDIUM / HIGH |
| implementation_effort | VARCHAR(20) | | 实施周期估计：DAYS / WEEKS / MONTHS |
| created_at | DATETIME | ✓ | 创建时间 |

#### PlanVersion（方案归档版本）
| 字段名 | 类型 | 必填 | 说明 |
|--------|------|------|------|
| version_id | VARCHAR(36) | ✓ | 主键 |
| plan_id | VARCHAR(36) | ✓ | 关联方案（外键→SimulationPlan） |
| version_name | VARCHAR(100) | ✓ | 版本名称（如"方案A-增加贴片机"） |
| version_no | INTEGER | ✓ | 版本序号（方案内自增） |
| is_baseline | BOOLEAN | ✓ | 是否为基线版本（对比基准，同方案仅一个为 TRUE） |
| key_metrics | JSON | | 关键指标快照（output_per_hour / lbr / bottleneck_utilization） |
| notes | TEXT | | 版本备注（改动说明） |
| archived_at | DATETIME | ✓ | 归档时间 |
| archived_by | VARCHAR(50) | | 归档操作人 |
| created_at | DATETIME | ✓ | 创建时间 |
